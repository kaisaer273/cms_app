...
class CartController extends GetxController {
  ...
  void onInit() {
    ...
    filteredMedicineList.value =
        MedicineModel.medicines; // Bắt đầu bằng việc hiển thị toàn bộ danh sách
  }
  ...
  void filterMedicine(String query) {
    ...
    filteredMedicineList.value = MedicineModel.medicines.where((item) {
      return item.activeIngredientName
              .toLowerCase()
  ...
  medicineInCartPrice() => (_medicinesInCart.isNotEmpty)
      ? _medicinesInCart.entries.map((e) => e.key.priceOut3 * e.value).toList()
      : 0;

  get totalPrice => (_medicinesInCart.isNotEmpty)
      ? _medicinesInCart.entries
          .map((e) => e.key.priceOut3 * e.value)
          .toList()
